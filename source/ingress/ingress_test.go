...
type IngressSuite struct {
	suite.Suite
	sc             Source
	fooWithTargets *extensionv1beta1.Ingress
}
...
func TestNewIngressSource(t *testing.T) {
	for _, ti := range []struct {
		...
		t.Run(ti.title, func(t *testing.T) {
			_, err := NewSource(
				fake.NewSimpleClientset().ExtensionsV1beta1(),
				"",
				ti.annotationFilter,
				ti.fqdnTemplate,
