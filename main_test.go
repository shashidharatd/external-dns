...
func (suite *ByNamesTestSuite) TestAllInitialized() {
	...
	sources, err := ByNames(mockClientGenerator, []string{"service", "ingress", "fake"}, &source.Config{})
...
func (suite *ByNamesTestSuite) TestOnlyFake() {
	...
	sources, err := ByNames(mockClientGenerator, []string{"fake"}, &source.Config{})
	suite.NoError(err, "should not generate errors")
...
func (suite *ByNamesTestSuite) TestSourceNotFound() {
	...
	sources, err := ByNames(mockClientGenerator, []string{"foo"}, &source.Config{})
...
func (suite *ByNamesTestSuite) TestKubeClientFails() {
	...
	_, err := ByNames(mockClientGenerator, []string{"service"}, &source.Config{})
	suite.Error(err, "should return an error if client cannot be created")

	_, err = ByNames(mockClientGenerator, []string{"ingress"}, &source.Config{})
	suite.Error(err, "should return an error if client cannot be created")
}
